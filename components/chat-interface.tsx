...
const AI_MODELS = [
  ...
  { id: "gemini-2.5-flash", name: "Gemini 2.5 Flash", provider: "Google AI Studio" },
];
...
export function ChatInterface() {
  ...
  const sendMessage = async (messageText: string) => {
    ...
    const userMessage: Message = {
      id: generateId(),
      role: "user",
      content: messageText.trim() || (currentImageUrl ? "Uploaded an image" : ""),
    ...
    try {
      ...
      const isRateLimit = error instanceof Error && error.message === "429_TOO_MANY_REQUESTS";
      const errorMessage: Message = {
        id: assistantMessageId,
        role: "assistant",
        content: isRateLimit
          ? "⚠️ The selected AI model is currently rate-limited or out of quota. Please switch to the other model in the dropdown above!"
          : `Sorry, I encountered an error. ${error.message?.includes('API Error') ? '\\n\\n**Details:** ' + error.message : 'Please try again!'}`,
        timestamp: new Date().toISOString(),
      };
  ...
  const saveEdit = async (messageId: string) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: generateId(),
        role: "assistant",
        content: error.message === "429_TOO_MANY_REQUESTS"
          ? "⚠️ The selected AI model is currently rate-limited or out of quota. Please switch to the other model in the dropdown above!"
      ...
      <div className="flex-1 flex flex-col min-w-0">
            ...
            <div key={msg.id} className="animate-fadeIn">
              {msg.role === "assistant" ? (
                <div className="max-w-3xl mx-auto group">
                  ...
                  {/* Message Bubble */}
                  <div className="glass-card rounded-2xl px-4 py-3 text-slate-200 text-sm leading-relaxed prose prose-invert prose-sm max-w-none relative">
                    {msg.imageUrl && (
        ...
        <div className="px-4 py-4">
          <div className="max-w-3xl mx-auto flex flex-col gap-2">
            ...
            {selectedImage && (
              <div className="flex items-center gap-3 px-1 animate-fadeIn">
                <div className="relative group w-16 h-16 rounded-xl overflow-hidden border-2 border-slate-700/50 hover:border-blue-500/50 shadow-sm transition-all bg-slate-800/80">
